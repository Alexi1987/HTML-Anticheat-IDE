...
public class Main extends JFrame {
    ...
    private ClientHandler clientHandler; // Adiciona uma referência a ClientHandler
    ...
    public Main() {
        ...
        banIPButton.addActionListener(e -> {
            ...
            if (ipToBan != null && !ipToBan.isEmpty()) {
                clientHandler.banIP(ipToBan); // Usa a referência para chamar o método
            }
        });
        ...
        banUserButton.addActionListener(e -> {
            ...
            if (userToBan != null && !userToBan.isEmpty()) {
                clientHandler.banUser(userToBan); // Usa a referência para chamar o método
            }
        });

        JButton showConnectedUsersButton = new JButton("Mostrar Conectados");
        showConnectedUsersButton.addActionListener(e -> clientHandler.showConnectedUsers()); // Usa a referência para chamar o método
    ...
    private void startServer() {
        clientHandler = new ClientHandler(); // Cria uma instância de ClientHandler
        Thread serverThread = new Thread(() -> {
            try {
                ...
                while (true) {
                    ...
                    clientHandler.setSocket(clientSocket); // Define o socket para a instância de ClientHandler
    ...
    private class ClientHandler implements Runnable {
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()))) {
                ...
                while ((line = reader.readLine()) != null) {
                    if (line.startsWith("Trapaça detectada por:")) {
                        ...
                        // Processar outras mensagens do cliente aqui, se necessário
        ...
        private void displayCheaterInfo(String message) {
            SwingUtilities.invokeLater(() -> cheaterInfoTextArea.append("[" + dateFormat.format(new Date()) + "] " + message + "\n"));
        }

        private void saveIndexHtml(String userName, String content) {
            if (!content.trim().isEmpty()) {
                // Verificar se o IP ou o usuário está banido
